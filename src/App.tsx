...
function App() {
  ...
  const {
    ram, hideLauncher, showConsole, installOptifine,
    getAccentStyles // Now from context
  } = useSettings();
  ...
  const handleSync = async () => {
    ...
    setLogs((prev) => [...prev, 'Starting modpack sync...']);

    // Subscribe to updater progress specifically
    const unsub = window.updater.onProgress(({ status, progress }) => {
      setStatusText(status);
      setProgress(progress);
    });

    try {
      // Point to local test server or use input
      const manifestUrl = 'http://localhost:3000/modpacks/example-pack/manifest.json';
      await window.updater.sync(manifestUrl);
  ...
  const getVersionHint = (v: string) => {
    ...
    if (v === '1.6.4') return { text: '✅ Verified Stable', ...accent };
    if (v === '1.12.2') return { text: '🏆 Classic Stable (Recommended)', ...accent };
    if (v === '1.7.10') return { text: '⭐ Legendary Stable', ...accent };
    ...
    if (v.startsWith('1.8') && v !== '1.8.9' && v !== '1.8.0') return { text: '❌ No Forge available', className: 'text-red-400' };
    if (v.startsWith('1.9') && major === 9 && minor <= 3) return { text: '❌ No Forge available', className: 'text-red-400' };
    if (v === '1.11.1') return { text: '❌ No Forge available', className: 'text-red-400' };

    if (major >= 13) return { text: '🚧 Forge support in development (New Installer)', className: 'text-yellow-400' };
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-zinc-900 text-zinc-100 overflow-hidden font-sans border border-zinc-800">
      <TitleBar />
      ...
      {/* Multiplayer Overlay */}
      {showMultiplayer && (
        <div className="absolute inset-0 z-50 bg-zinc-900/95 flex items-center justify-center p-10 backdrop-blur-sm">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        <div className="w-80 flex flex-col p-6 bg-zinc-800 border-r border-zinc-700 shadow-xl z-10 relative">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <p className="text-[10px] text-zinc-500 font-mono mt-1">BUILD v0.2.0-BETA</p>
            </div>
          ...
          <div className="space-y-5 flex-1 flex flex-col pt-2">
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider">Nickname</label>
                {isOffline && (
                  <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-amber-900/50 text-amber-500 border border-amber-800/50 uppercase tracking-wider">
                    Offline Mode
                  </span>
                )}
              </div>
              <input
                ...
                className={`w-full bg-zinc-900 border border-zinc-700 rounded p-3 focus:outline-none transition-colors placeholder-zinc-600 ${getAccentStyles('border').className || ''}`}
            ...
            <div>
              <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Version</label>
              <select
                value={version}
                onChange={(e) => setVersion(e.target.value)}
                className={`w-full bg-zinc-900 border border-zinc-700 rounded p-3 focus:outline-none transition-colors ${getAccentStyles('border').className || ''}`}
                style={getAccentStyles('border').style}
              >
                {versions.map(v => (
                  <option key={v.id} value={v.id}>{v.id}</option>
                ))}
                <option value="Example Pack">Example Modpack</option>
              </select>
              {currentHint && (
                <p
            ...
            <div className="flex items-center space-x-3 bg-zinc-900 p-3 rounded border border-zinc-700">
              <input
                type="checkbox"
                id="forge-toggle"
                checked={useForge}
              ...
              />
              <label htmlFor="forge-toggle" className="text-sm text-zinc-300 cursor-pointer select-none">
                Enable Forge ModLoader
              </label>
            </div>

            {version === 'Example Pack' && (
              <div className="p-2 bg-zinc-700/50 rounded border border-zinc-600/50 mt-4">
                <p className="text-xs text-zinc-400 mb-2">Development Mode</p>
                <button
                  onClick={handleSync}
                  disabled={isLaunching}
                  className="w-full py-2 bg-blue-600 hover:bg-blue-500 rounded text-xs font-bold uppercase tracking-wider"
                >
                  Sync Modpack
                </button>
              </div>
            ...
            <div className="flex gap-2">
              ...
              {isLaunching && (
                <button
                  ...
                  className="px-4 bg-red-900/50 hover:bg-red-800 text-red-200 rounded font-bold text-xs uppercase"
        ...
        <div className="flex-1 flex flex-col bg-zinc-950 min-w-0 transition-all duration-300">
          {showConsole ? (
            <div className="flex flex-col h-full p-6">
              <div className="flex justify-between items-center mb-4 flex-shrink-0">
                ...
                <button onClick={copyLogs} className="text-xs text-zinc-500 hover:text-zinc-300 transition-colors">Copy</button>
              </div>
              <div className="flex-1 bg-zinc-900/50 rounded-lg border border-zinc-800/50 p-4 font-mono text-xs text-zinc-400 overflow-y-auto custom-scrollbar shadow-inner">
                {logs.length === 0 && <div className="h-full flex items-center justify-center text-zinc-700 opacity-50">Waiting for game...</div>}
                {logs.map((log, i) => (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-zinc-500 p-10 select-none">
              <img src="/tray-icon.png" className="w-32 h-32 opacity-80 mb-4" />
              <p className="text-3xl font-black uppercase tracking-widest text-zinc-700">FriendLauncher</p>
            </div>
          )}
        </div>
      </div>
