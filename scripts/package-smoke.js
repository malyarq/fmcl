...
export async function runPackageSmoke(options = {}) {
  ...
  try {
    ...
    await runtime.waitForRendererReadiness(
      debugPort,
      options.readinessTimeoutMs ?? READY_TIMEOUT_MS,
      child,
    ).then((pages) => { evidence.launch.readiness = 'remote-debugging-page'; evidence.launch.windowCount = pages.length; });
    evidence.quit.requested = true;
    runtime.requestGracefulQuit({ platform, child });
