...
function dependencies() {
  ...
  return {
    ...
    burrowLink: { getState: vi.fn(), host: vi.fn(), join: vi.fn(), stop: vi.fn(), subscribe: vi.fn(() => vi.fn()) },
...
describe('network IPC handlers', () => {
  ...
  it('routes validated requests to independent owners', async () => {
    ...
    expect(deps.burrowLink.host).toHaveBeenCalledWith(25_565);
  ...
  it('forwards typed owner events and removes exact subscriptions with the window', () => {
    ...
    deps.burrowLink.subscribe.mockReturnValue(disposers[0]);
    ...
    registerNetworkHandlers(deps as never);
    const tunnelListener = (deps.burrowLink.subscribe.mock.calls as unknown as Array<[(value: unknown) => void]>)[0][0];
